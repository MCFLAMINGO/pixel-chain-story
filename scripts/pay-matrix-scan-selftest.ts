 ...
 * No canvas — proves the camera crop path used by Scan matrix.
 */
import { encodePayFaceMatrix } from "../src/lib/pixel/pay-face-optical";
import { patternToRaster, type PixelRaster } from "../src/lib/pixel/optical-capture";
import {
  findBrightRect,
  pollPayMatrixRaster,
  squareifyRect,
...
const square = patternToRaster(pattern, 16);
...
// Letterboxed like a tall phone camera frame
{
  ...
  assert(Math.abs(sq.w - square.width) < 20, `squareify side ~${square.width} got ${sq.w}`);
  const hit = await pollPayMatrixRaster(framed);
  assert(hit.locked && hit.address === addr, `letterbox decode: ${JSON.stringify(hit)}`);
}

// Warm phosphor tint (projector CSS) + letterbox
{
...
// Small face in a large black frame (friend farther away)
{
  const small = patternToRaster(pattern, 8);
