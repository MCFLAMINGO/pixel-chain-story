...
3. [~] **Canonical tip feed** — host recipe (`tip:host`, `Dockerfile.tip`, [`CANONICAL-TIP.md`](./CANONICAL-TIP.md)); always-on tip at `https://pixel-tip-production.up.railway.app`; people pay via `POST /tx` → `shared_tip` (`test:shared-tip`). Still open: Lovable `VITE_PIXEL_RPC` (+ `VITE_REQUIRE_PUBLIC_TIP=1`) so `/` shows **public tip** (ops, not more invent).
...
6. [~] **Proximity / discovery** — lattice Chebyshev peers in tip FieldWitness (S1, [`SPATIAL.md`](./SPATIAL.md)); later optical / network discovery. Brightness as activity truth, not fake physics.
