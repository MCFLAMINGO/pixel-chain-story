...
Pixel has 85 modules under `src/lib/pixel`. **Twenty-one of them are the protocol.** The
...
These twenty-one are reachable from `acceptBlock`, `verifyChain` or `sequenceBlock`. A bug
...
| `finality.ts`        | What cannot revert: anchored checkpoints (off unless enabled)   |
