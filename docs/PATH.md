...
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1–S3 tip geometry/wave/picture; S4 fan-out + hash-grid index + named damping + partition sim (`test:wave-fanout`, `test:spatial-index`, `test:wave-partition`). Still open: LockFeeder→lead invent, S5 WASM/Three.js/octree-if-benched. Dream ≠ voxel mainnet claim.
