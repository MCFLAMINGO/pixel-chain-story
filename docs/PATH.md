...
7. [~] **World canvas + people wallet** — frame + lab `/wallet` pay-on-tip + billboard honesty + tip-mark / canvas id + shared-tip attach (`test:wallet`, `test:tip-mark`, `test:shared-tip`, [`WORLD-CANVAS.md`](./WORLD-CANVAS.md), [`CANONICAL-TIP.md`](./CANONICAL-TIP.md)). Still open: hosted public tip as production default. Lab `init` stays for builders only.
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1 lattice + blend (`test:lattice`); S2 tip-bound lead waves + `waveDigest` (`test:wave`); S3 sparse occupancy Merkle + `spatialRoot` + cell proofs (`test:spatial-proof`); S4 wave fan-out notify plane (`test:wave-fanout`). Still open: S4 scale index / damping, S5 WASM/Three.js sink. Dream ≠ voxel mainnet claim.
