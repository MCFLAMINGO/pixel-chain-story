...
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1 lattice + blend (`test:lattice`); S2 tip-bound lead waves + `waveDigest` (`test:wave`); S3 sparse occupancy Merkle + `spatialRoot` + cell proofs (`test:spatial-proof`). Still open: S4 scale / damping / async fan-out, S5 WASM/Three.js sink. Dream ≠ voxel mainnet claim.
