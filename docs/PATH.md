...
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1–S3 tip geometry/wave/picture; S4 fan-out (`test:wave-fanout`); Lock→lead invent (`test:lock-lead`). Still open / parallel: S4 index+damping+partition PR; S5 WASM/Three.js. Dream ≠ voxel mainnet claim.
