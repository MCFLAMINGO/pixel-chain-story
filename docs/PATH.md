...
7. [~] **World canvas + people wallet** — frame + lab `/wallet` pay-on-tip + billboard honesty + tip-mark / canvas id + shared-tip attach (`test:wallet`, `test:tip-mark`, `test:shared-tip`); tip-host contract + `VITE_REQUIRE_PUBLIC_TIP` (`test:tip-host`, [`CANONICAL-TIP.md`](./CANONICAL-TIP.md)). Still open: **ops** — durable public tip URL wired into production `VITE_PIXEL_RPC`. Lab `init` stays for builders only.
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1–S3 tip geometry/wave/picture; S4 fan-out + hash-grid index + named damping + partition sim (`test:wave-fanout`, `test:spatial-index`, `test:wave-partition`); Lock→lead invent (`test:lock-lead`). Still open: S5 WASM/Three.js/octree-if-benched. Dream ≠ voxel mainnet claim.
