...
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1 lattice Chebyshev + weighted blend in `fieldDigest` (`test:lattice`, deepened `test:field`). Still open: S2 tip-bound waves, S3 spatial Merkle, S4 scale. Dream ≠ voxel mainnet claim.
