...
8. [~] **Spatial lattice invent** — meat behind the 3D/light vision ([`SPATIAL.md`](./SPATIAL.md)): S1 lattice + blend (`test:lattice`); S2 tip-bound lead waves + `waveDigest` in PoLS (`test:wave`). Still open: S3 spatial Merkle, S4 scale. Dream ≠ voxel mainnet claim.
