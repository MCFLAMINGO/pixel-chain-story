...
7. [~] **World canvas + people wallet** — frame + lab `/wallet` + billboard honesty + tip-mark receipts + canvas id (`test:wallet`, `test:tip-mark`, [`WORLD-CANVAS.md`](./WORLD-CANVAS.md)). Still open: default canonical public tip (`shared_tip` by default). Lab `init` stays for builders only.
