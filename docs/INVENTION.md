...
| **World canvas**             | One public picture of humanity — people wallets + tip marks; `CanvasId=(networkId,genesisHash)`; `shared_tip` via `POST /tx`; operators light one field — not mass `init` ([`WORLD-CANVAS.md`](./WORLD-CANVAS.md), [`CANONICAL-TIP.md`](./CANONICAL-TIP.md)). |
